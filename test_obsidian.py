...
> Treat it as a reference.""") << rep(newline)
    ...
    typed_texts = (
        typed_text_title
        >> rep(newline)
        >> until((newline >> h3_tag) | (newline >> h4_tag) | eof)
        << rep(newline)
    )
    ...
    document = (
        frontmatter & document_title << autogeneration_warning & pages
    ) > to_document
...
def test_tags_present_on_a_page_are_in_the_markdown(
    notebook: NotebookMetadata,
    remarks_document: Document,
    ...
    if obsidian_markdown:
        ...
        for page in notebook.pages:
            if page.merged_highlights:
                ...
                assert (
                    len1 == len2
                ), f"The length of the parsed highlights {len1} differs from the length of the expected highlights {len2}"
