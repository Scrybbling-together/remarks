...
from .conversion.parsing import parse_rm_file
...
def process_document(
    ...
    # Load doc with rm_api too
    document__new = LocalDocument.load_rmdoc(metadata_path.parent)
    ...
    for (
        ...
        # Load the document with librm_lines for obsidian TODO: This is temporary implementation for obsidian only
        try:
            tree = pylibrm_lines.SceneTree.from_document(document__new, page_uuid)
        except FailedToBuildTree:
            # Handle the case where the scene tree could not be built for this page
            # We handle this early with librm_lines,
            # Cause if it can't read it, nothing else can
            # This removes the redundant part below,
            # which allows skipping straight into the rest of remarks process
            scrybble_warning_tree_failed_to_build.render_as_annotation(page)
            continue
        renderer = pylibrm_lines.Renderer(tree)

        # Add the text
        if renderer.paragraphs:
            obsidian_markdown.add_text(page_uuid, renderer.paragraphs)

        # Disable line output and grab glyph ranges
        renderer.config.use_whitelist = True
        renderer.config.follow_rules_in_json = True
        glyph_ranges = [
            glyph_range_info.glyph_range
            for layer in (renderer.layers or [])
            for glyph_range_info in layer.glyph_ranges
        ]
        obsidian_markdown.add_highlights(page_uuid, glyph_ranges)
